...
option_list <- list(
  ...
  make_option(c("-x", "--exclude-dir"), dest = "exclude_dir", type = "character", action = "append", default = c("_mqc_inputs")),
...
if (!is.null(opt$target_genes)) {
  ...
  message("DEBUG: No target gene files received.")
}
...
find_files <- function(dir) {
  list.files(dir, pattern="^abundance\\.tsv$", recursive=TRUE, full.names=TRUE)
}
...
make_plots <- function(vst_mat, info) {
  ...
  # Align
  info <- info[match(colnames(vst_mat), info$sample),]
  ...
  draw_hm <- function(m, t, f) {
    ...
    ComplexHeatmap::draw(ComplexHeatmap::Heatmap(m, name="vst",
      show_row_names=(nrow(m)<100), show_column_names=FALSE,
      column_split=grp_vec, column_title=t))
    grDevices::dev.off()
    msg("[Heatmap] Saved %s to %s", t, f)
  }
  ...
  if(DO_HEATMAP && !is.null(targets_vec)) {
    ...
    for(i in seq_along(targets_vec)) {
      ...
      if(length(idx) > 0) {
        ...
        msg("[TargetPlot] WARNING: No matching genes for %s", bases[i])
      }
    }
  }
  ...
  draw_var_hm <- function(genes, t, f) {
    ...
    grDevices::pdf(f, width = 8, height = 6)
    ComplexHeatmap::draw(ComplexHeatmap::Heatmap(var_mat, name="var(VST)", show_row_names=(nrow(var_mat)<100)))
    grDevices::dev.off()
    msg("[VarHeatmap] Saved %s to %s", t, f)
  }

  if(DO_VAR_HM) {
    # 1. Global Variance Heatmap
    msg("[VarHeatmap] Generating Global Variance Heatmap")
    draw_var_hm(rownames(vst_top), "Global Variance", var_hm_pdf)

    # 2. Targeted Variance Heatmaps (NEW)
...
if(PLOTS_ONLY) {
  ...
  info <- info[info$sample %in% common, ]
...
# Pipeline
txi <- if(file.exists(txi_rds) && !opt$force_txi) readRDS(txi_rds) else {
  tximport(files, type="kallisto", tx2gene=read_tx2gene(TX2GENE_PATH),
